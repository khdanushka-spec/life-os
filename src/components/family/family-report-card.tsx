...
import { generateFamilyReportAction } from "@/server/actions/family";
...
export function FamilyReportCard({
  period,
  periodStart,
  initialReport,
}: {
  period: ReportPeriod;
  periodStart: string;
  initialReport: FamilyReportSummary | null;
}) {
  ...
  async function generate() {
    setLoading(true);
    const result = await generateFamilyReportAction(period, periodStart);
        ...
        <CardContent className="flex flex-col gap-3 text-sm">
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="rounded-lg border p-2">
              ...
              <p className="font-semibold tabular-nums">{report.eventsInPeriod}</p>
            </div>
            <div className="rounded-lg border p-2">
              <p className="text-[11px] text-muted-foreground">Birthdays</p>
              <p className="font-semibold tabular-nums">{report.birthdaysInPeriod}</p>
            </div>
            <div className="rounded-lg border p-2">
              <p className="text-[11px] text-muted-foreground">Gifts Given</p>
              <p className="font-semibold tabular-nums">{report.giftsGiven}</p>
            </div>
          </div>
