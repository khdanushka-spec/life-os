...
import { LearningReportCard } from "@/components/learning/learning-report-card";
...
export default async function LearningReportsPage() {
  ...
  const reports = await Promise.all(
    periods.map(({ period, periodStart }) =>
      prisma.learningReport.findUnique({
        ...
        {periods.map(({ period, periodStart }, i) => (
          <TabsContent key={period} value={period}>
            <LearningReportCard
              ...
              initialReport={(reports[i]?.summary as LearningReportSummary | undefined) ?? null}
            />
          </TabsContent>
        ))}
      </Tabs>
