...
import { TravelReportCard } from "@/components/travel/travel-report-card";
...
export default async function TravelReportsPage() {
  ...
  const reports = await Promise.all(
    periods.map(({ period, periodStart }) =>
      prisma.travelReport.findUnique({
        ...
        {periods.map(({ period, periodStart }, i) => (
          <TabsContent key={period} value={period}>
            <TravelReportCard
              ...
              initialReport={(reports[i]?.summary as TravelReportSummary | undefined) ?? null}
            />
          </TabsContent>
        ))}
      </Tabs>
